...
//        System.out.println(wordNetAll);
    ...
    public List<Term> seg(String text)
    {
        ...
        System.out.println("charArray的长度:"+charArray.length);
        System.out.println(charArray);
        if (HanLP.Config.Normalization)
        {
            CharTable.normalization(charArray);
            //System.out.println("Normalization:"+"true");//没有被执行
        }
        //config.threadNumber在Config.java中定义为1
        if (config.threadNumber > 1 && charArray.length > 10000)    // 小文本多线程没意义，反而变慢了
        {
        	System.out.println("charArray长度大于10000");//length小于10000时没有被执行
            ...
            if (config.indexMode)
            {
                //int sentenceOffset = 0;//offset
                for (int i = 0; i < sentenceArray.length; ++i)
                {
                    for (Term term : termListArray[i])
                    {
                        //term.offset += sentenceOffset;//offset
                        termList.add(term);
                    }
                    //sentenceOffset += sentenceArray[i].length();//offset
                }
            }
            else
            {
