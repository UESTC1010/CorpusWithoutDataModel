...
public class TextRankKeyword extends KeywordExtractor
{
    ...
    {
        Map<String, Float> map = getTermAndRank(content);//map为全部分词及对应的rank
    ...
    public Map<String,Float> getRank(List<Term> termList)
    {
        ...
        }////去除停用词功能,此后wordList为不包含停用词的分词结果。
        System.out.println("去除停用词后:"+wordList);//
        Map<String, Set<String>> words = new TreeMap<String, Set<String>>();
        Queue<String> que = new LinkedList<String>();
        for (String w : wordList)
        {
        	//向words中添加key,value暂时为空
            if (!words.containsKey(w))
            {
                words.put(w, new TreeSet<String>());
            }
            que.offer(w);
            //建立大小为5的窗口，每个单词将投票给它身前身后距离为5以内的单词
            if (que.size() > 5)
            {
                que.poll();
            }
            ...
            for (String w1 : que)
            {
                for (String w2 : que)
                {
                    ...
                    //生成words的value部分
                    words.get(w1).add(w2);//获得以w1为key的Set<String>,并向Set<String>中添加w2
                    words.get(w2).add(w1);
                }
            }
        }
        ...
        for (int i = 0; i < max_iter; ++i)
        {
            ...
            for (Map.Entry<String, Set<String>> entry : words.entrySet())
            {
                ...
                {
                    int size = words.get(element).size();//map.get(key)返回指定键所映射的
                    if (key.equals(element) || size == 0) continue;
                    m.put(key, m.get(key) + d / size * (score.get(element) == null ? 0 : score.get(element)));
                    //m.put相同的key,value会被覆盖
                }
                //Math.abs(取绝对值)
